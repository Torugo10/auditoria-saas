...
from backend.modules.suporte import (
...
SESSION_COOKIE_NAME = "auditoria_session"
...
def formatar_data_hora_local(series):
    """Formata timestamps salvos em UTC para o fuso da aplicacao."""
    return (
        pd.to_datetime(series, errors="coerce", utc=True)
        .dt.tz_convert(ZoneInfo(settings.app_timezone))
...
def inicializar_banco_dados():
    ...
    try:
        ...
        c.execute("ALTER TABLE administradores ADD COLUMN IF NOT EXISTS cnpj_cpf TEXT")
        c.execute("ALTER TABLE gerentes ADD COLUMN IF NOT EXISTS nome_empresa TEXT")
        c.execute("ALTER TABLE usuarios_finais ADD COLUMN IF NOT EXISTS nome TEXT")
        c.execute("ALTER TABLE contas_recorrentes ADD COLUMN IF NOT EXISTS cnpj_cpf_gerente TEXT")
        c.execute("ALTER TABLE contas_recorrentes ADD COLUMN IF NOT EXISTS login_usuario TEXT")
        c.execute("ALTER TABLE contas_recorrentes ADD COLUMN IF NOT EXISTS cnpj_cpf_usuario TEXT")
        c.execute("ALTER TABLE aliquotas_padrao ADD COLUMN IF NOT EXISTS aliquota_icms_esperada REAL")
        c.execute("ALTER TABLE aliquotas_padrao ADD COLUMN IF NOT EXISTS aliquota_pis_esperada REAL")
        c.execute("ALTER TABLE aliquotas_padrao ADD COLUMN IF NOT EXISTS aliquota_cofins_esperada REAL")
        c.execute("ALTER TABLE aliquotas_padrao ADD COLUMN IF NOT EXISTS aliquota_ipi_esperada REAL")
        c.execute("ALTER TABLE aliquotas_padrao ADD COLUMN IF NOT EXISTS regime TEXT DEFAULT 'Simples Nacional'")
        c.execute("ALTER TABLE aliquotas_padrao ADD COLUMN IF NOT EXISTS anexo TEXT DEFAULT 'III'")

        if settings.bootstrap_admin_login and settings.bootstrap_admin_password:
            c.execute("SELECT COUNT(*) FROM administradores")
            admin_count = c.fetchone()[0]
            if admin_count == 0:
                senha_hash = hashlib.sha256(settings.bootstrap_admin_password.encode()).hexdigest()
                c.execute('''
                          INSERT INTO administradores (login, senha_hash, email, nome_completo, ativo)
                          VALUES (?, ?, ?, ?, 1)
                          ''', (
...
def _ensure_db_initialized():
    """Initializes the database once per Streamlit session."""
...
def restaurar_sessao_por_token():
    """Restaura login apos refresh, validando assinatura e usuario ativo."""
    if logout_marcado():
        clear_authenticated_user(st.session_state)
    ...
    token = obter_cookie_sessao()
    token_query_string = st.query_params.get("sid")
    if isinstance(token_query_string, list):
        token_query_string = token_query_string[0] if token_query_string else None

    if token_query_string:
        token = token_query_string
        salvar_cookie_sessao(token_query_string)
        limpar_token_url()

    usuario_token = parse_session_token(token, settings.secret_key)
    ...
    try:
        sessao_ativa = validate_active_session(
            ...
            login=usuario_token.login,
        )
    except Exception:
        sessao_ativa = False

    if sessao_ativa:
        apply_authenticated_user(st.session_state, usuario_token)
...
def salvar_token_sessao(usuario):
...
def sincronizar_cookie_sessao():
    token = st.session_state.get("session_token")
    if st.session_state.autenticado and token:
        salvar_cookie_sessao(token)


def limpar_token_url():
...
def logout_marcado():
    ...
    if isinstance(logout, list):
        logout = logout[0] if logout else None
    return logout == "1"
...
def salvar_cookie_sessao(token):
    renderizar_script_cookie(
        f"{SESSION_COOKIE_NAME}={token}; Max-Age={settings.session_ttl_seconds}; Path=/; SameSite=Lax"
    )


def limpar_cookie_sessao(reload_page=False):
    renderizar_script_cookie(
        f"{SESSION_COOKIE_NAME}=; Max-Age=0; Path=/; SameSite=Lax",
        reload_page=reload_page,
    )


def renderizar_script_cookie(cookie_value, reload_page=False):
    reload_script = ""
    if reload_page:
        reload_script = """
            setTimeout(function() {
    ...
    components.html(
        f"""
        <script>
            document.cookie = {cookie_value!r};
            {reload_script}
        </script>
        """,
        height=0,
        ...
        if not c.fetchone():
            ...
            for cfop, desc, icms, pis, cofins, ipi in dados_aliquotas:
                c.execute('''
                          ...
                          (cfop, descricao, aliquota_icms_esperada, aliquota_pis_esperada, aliquota_cofins_esperada,
                           aliquota_ipi_esperada)
                          VALUES (?, ?, ?, ?, ?, ?)
...
initialize_support_schema(database_url=DATABASE_URL, database_path=DB_PATH)
...
def localizar_cabecalho_real(df, chaves_principais, chaves_alternativas=None):
    ...
    for i in range(limite):
        ...
        if all(any(palavra.lower() in celula for celula in linha) for palavra in chaves_principais):
            ...
            return novo_df
        if chaves_alternativas and all(
                any(palavra.lower() in celula for celula in linha) for palavra in chaves_alternativas):
            novo_df = df.iloc[i + 1:].copy()
            novo_df.columns = df.iloc[i].values
            novo_df.columns = [str(col).strip() for col in novo_df.columns]
            return novo_df
...
def parser_cabecalho_dominio(df_raw):
    ...
    if linha_macro != -1:
        ...
        return df_clean.dropna(subset=[col for col in df_clean.columns if 'cfop' in str(col) or 'ncm' in str(col)],
                               thresh=1)
    else:
...
def parser_cabecalho_saida(df_raw):
    ...
    for i in range(min(10, len(df_raw))):
        linha_str = df_raw.iloc[i].astype(str).str.lower().tolist()
        if any('icms' in val for val in linha_str) and any('pis' in val for val in linha_str) and any(
                'cofins' in val for val in linha_str):
            linha_macro = i
            linha_detalhe = i + 1
            break
...
def app_auditoria_itens_entrada():
    ...
    arquivo_itens = st.file_uploader("Upload", type=['csv', 'xlsx', 'xls'], key="up_itens",
    ...
    if arquivo_itens:
        with st.spinner("Compilando árvore hierárquica..."):
            ...
            if df_raw is not None:
                ...
                df['Descricao'] = col_descricao.astype(
                    str).str.strip() if col_descricao.name != "ND" else "Não Informado"
                df['NCM'] = col_ncm.astype(str).str.replace('.0', '',
                                                            regex=False).str.strip() if col_ncm.name != "ND" else "Não Informado"
                ...
                df['CST_ICMS'] = pegar_coluna_todas_chaves(df, ['icms normal', 'cst']).astype(str).str.replace('.0', '',
                ...
                df['CST_PIS'] = pegar_coluna_todas_chaves(df, ['pis', 'cst']).astype(str).str.replace('.0', '',
                                                                                                      regex=False).str.strip()
                ...
                df['CST_COFINS'] = pegar_coluna_todas_chaves(df, ['cofins', 'cst']).astype(str).str.replace('.0', '',
                                                                                                            regex=False).str.strip()
                ...
                def renderizar_drill_tributario(nome_imposto, sufixo_imposto):
                    ...
                    df_resumo = df.groupby(['CFOP_Clean', col_cst_imposto]).agg(
                        {'Total_Liquido_Produto': 'sum'}).reset_index()
                    ...
                    st.dataframe(df_resumo[['CFOP', 'CST', 'Total_Formatado']], width='stretch')

                    col_exp1, col_exp2 = st.columns([4, 1])
                    with col_exp2:
                        excel_consolidado = exportar_excel_formatado(
                            df_resumo[['CFOP', 'CST', 'Total_Liquido_Produto']], f"Consolidado_{sufixo_imposto}")
                        st.download_button(
                            label="📥 Excel",
                    ...
                    lista_grupos = df_resumo.apply(lambda row: f"CFOP: {row['CFOP']} | CST: {row['CST']}",
                                                   axis=1).tolist()
                    if lista_grupos:
                ...
                if visao == "🟦 ICMS Normal":
                    renderizar_drill_tributario("ICMS Normal", "ICMS")
                elif visao == "🟨 PIS":
                    renderizar_drill_tributario("PIS", "PIS")
                elif visao == "🟩 COFINS":
                    renderizar_drill_tributario("COFINS", "COFINS")
...
def app_auditoria_itens_saida():
    ...
    arquivo_itens = st.file_uploader("Upload", type=['csv', 'xlsx', 'xls'], key="up_itens_saida",
                                     label_visibility="collapsed")
    ...
    if arquivo_itens:
        with st.spinner("Compilando árvore hierárquica..."):
            ...
            if df_raw is not None:
                ...
                col_desconto = pegar_coluna_qualquer_chave(df, ['desconto', 'vlr desc'])
                col_desp = pegar_coluna_qualquer_chave(df,
                                                       ['desp ass', 'desp aces', 'despesas acessórias', 'acréscimo'])
                ...
                df['CST_ICMS'] = pegar_coluna_todas_chaves(df, ['icms', 'cst']).astype(str).str.replace('.0', '',
                                                                                                        regex=False).str.strip()
    ...
    if f_dominio and f_dte:
        with st.spinner('Cruzando dados...'):
            ...
            if df_dom_raw is not None and df_dte_raw is not None:
                ...
                df_dte['Chave_Acesso_DTE'] = c_chave_dte.astype(str).replace('nan',
                                                                             'Não Informada') if c_chave_dte.name != "ND" else "Não Informada"

                df_dte_grouped = df_dte.groupby('Chave_Nota').agg(
                    {'Valor_DTE': 'sum', 'Chave_Acesso_DTE': 'first'}).reset_index()

                merged = pd.merge(df_dom_grouped, df_dte_grouped, on='Chave_Nota', how='outer').fillna(
                    {'Valor_Dominio': 0, 'Valor_DTE': 0, 'Chave_Acesso_DTE': 'N/A'})
                merged['Diferença_Bruta'] = merged['Valor_Dominio'] - merged['Valor_DTE']
                merged['Diferença_Ajustada'] = merged.apply(
                    lambda r: 0.0 if abs(r['Diferença_Bruta']) <= tolerancia else r['Diferença_Bruta'], axis=1)

                d_val = merged[
                    (merged['Diferença_Ajustada'] != 0) & (merged['Valor_Dominio'] != 0) & (merged['Valor_DTE'] != 0)]
                f_dte = merged[(merged['Valor_DTE'] == 0) & (merged['Valor_Dominio'] != 0)]
                f_dom = merged[(merged['Valor_Dominio'] == 0) & (merged['Valor_DTE'] != 0)]

                t1, t2, t3 = st.tabs([f"⚠️ Divergência ({len(d_val)})", f"❌ Faltam DTE ({len(f_dte)})",
                                      f"❗ Faltam Dominio ({len(f_dom)})"])
                ...
                with t1:
                    if not d_val.empty:
                        ...
                        st.dataframe(v_diff_display[['N° Nota', 'Valor_Dominio', 'Valor_DTE', 'Diferença_Bruta']],
                                     width='stretch')

                        col_exp1, col_exp2 = st.columns([4, 1])
                        with col_exp2:
                            excel_div = exportar_excel_formatado(
                                v_diff[['N° Nota', 'Valor_Dominio', 'Valor_DTE', 'Diferença_Bruta']], "Divergências")
                            st.download_button(
                                label="📥 Excel",
                                data=excel_div,
                ...
                with t2:
                    if not f_dte.empty:
                        ...
                        st.dataframe(v_fdte_display[['N° Nota', 'Valor_Dominio']], width='stretch')
                ...
                with t3:
                    if not f_dom.empty:
                        v_fdom = f_dom.copy().rename(
                            columns={'Chave_Nota': 'N° Nota', 'Chave_Acesso_DTE': 'Chave da Nota'})
                        v_fdom_display = v_fdom.copy()
                        v_fdom_display['Valor_DTE'] = v_fdom_display['Valor_DTE'].apply(formata_br)
                        st.dataframe(v_fdom_display[['N° Nota', 'Chave da Nota', 'Valor_DTE']], width='stretch')

                        col_exp5, col_exp6 = st.columns([4, 1])
                        with col_exp6:
                            excel_fdom = exportar_excel_formatado(v_fdom[['N° Nota', 'Chave da Nota', 'Valor_DTE']],
                                                                  "Faltam_Dominio")
                            st.download_button(
                                label="📥 Excel",
                                data=excel_fdom,
    ...
    if f_dominio_novo and f_dte_novo:
        with st.spinner('Cruzando dados...'):
            ...
            if df_dom_novo_raw is not None and df_dte_novo_raw is not None:
                ...
                df_dte_novo = localizar_cabecalho_real(df_dte_novo_raw, ['número do documento fiscal', 'valor total'],
                                                       ['numero documento', 'valor'])
                ...
                c_nota_dte_novo = pegar_coluna_qualquer_chave(df_dte_novo,
                                                              ['número do documento fiscal', 'numero documento', 'nf-e',
                                                               'nfe', 'nota'])
                ...
                merged_novo = pd.merge(df_dom_novo_grouped, df_dte_novo_grouped, on='Chave_Nota', how='outer').fillna(
                    {'Valor_Dominio': 0, 'Valor_DTE': 0})
                merged_novo['Diferença_Bruta'] = merged_novo['Valor_Dominio'] - merged_novo['Valor_DTE']
                merged_novo['Diferença_Ajustada'] = merged_novo.apply(
                    lambda r: 0.0 if abs(r['Diferença_Bruta']) <= tolerancia_novo else r['Diferença_Bruta'], axis=1)

                d_val_novo = merged_novo[
                    (merged_novo['Diferença_Ajustada'] != 0) & (merged_novo['Valor_Dominio'] != 0) & (
                                merged_novo['Valor_DTE'] != 0)]
                ...
                t4, t5, t6 = st.tabs([f"⚠️ Divergência ({len(d_val_novo)})", f"❌ Faltam DTE ({len(f_dte_novo)})",
                                      f"❗ Faltam Dominio ({len(f_dom_novo)})"])
                ...
                with t4:
                    if not d_val_novo.empty:
                        ...
                        v_diff_novo_display['Diferença_Bruta'] = v_diff_novo_display['Diferença_Bruta'].apply(
                            formata_br)
                        st.dataframe(v_diff_novo_display[['N° Nota', 'Valor_Dominio', 'Valor_DTE', 'Diferença_Bruta']],
                                     width='stretch')

                        col_exp7, col_exp8 = st.columns([4, 1])
                        with col_exp8:
                            excel_div_novo = exportar_excel_formatado(
                                v_diff_novo[['N° Nota', 'Valor_Dominio', 'Valor_DTE', 'Diferença_Bruta']],
                                "Divergências")
                            st.download_button(
                                label="📥 Excel",
                                data=excel_div_novo,
                ...
                with t5:
                    if not f_dte_novo.empty:
                        ...
                        st.dataframe(v_fdte_novo_display[['N° Nota', 'Valor_Dominio']], width='stretch')

                        col_exp9, col_exp10 = st.columns([4, 1])
                        with col_exp10:
                            excel_fdte_novo = exportar_excel_formatado(v_fdte_novo[['N° Nota', 'Valor_Dominio']],
                                                                       "Faltam_DTE")
                            st.download_button(
                                label="📥 Excel",
                ...
                with t6:
                    if not f_dom_novo.empty:
                        ...
                        st.dataframe(v_fdom_novo_display[['N° Nota', 'Valor_DTE']], width='stretch')

                        col_exp11, col_exp12 = st.columns([4, 1])
                        with col_exp12:
                            excel_fdom_novo = exportar_excel_formatado(v_fdom_novo[['N° Nota', 'Valor_DTE']],
                                                                       "Faltam_Dominio")
                            st.download_button(
                                label="📥 Excel",
...
def app_painel_gerente():
    ...
    with tab1:
        ...
        st.info(f"ℹ️ O usuário será vinculado ao CNPJ/CPF **{st.session_state.cnpj_cpf}**")

        with st.form("form_usuario_final"):
            col_nome, col_email = st.columns(2)
            with col_nome:
                nome = st.text_input("Nome completo", placeholder="Nome do usuário")
            with col_email:
                email = st.text_input("Email", placeholder="usuario@empresa.com")

            col_login, col_senha = st.columns(2)
            with col_login:
                login = st.text_input("Login", placeholder="usuario.final")
            with col_senha:
                senha = st.text_input("Senha", type="password", placeholder="Mínimo 6 caracteres")

            col_perfil, col_submit = st.columns([2, 1])
            with col_perfil:
                perfil = st.selectbox("Perfil", ["analista", "supervisor", "auditor"])
            with col_submit:
                st.write("")
                st.write("")
                submit_usuario = st.form_submit_button("Criar usuário", width='stretch')
    ...
    with tab2:
        ...
        if usuarios:
            df_usuarios = pd.DataFrame(usuarios, columns=[
                'id', 'login', 'nome', 'email', 'perfil', 'criado_em', 'ativo'
            ])
            df_display = df_usuarios.copy()
            df_display["criado_em"] = formatar_data_hora_local(df_display["criado_em"])
            df_display["perfil"] = df_display["perfil"].fillna("").str.replace("_", " ").str.title()
            df_display["ativo"] = df_display["ativo"].apply(
                lambda valor: "Ativo" if int(valor) == 1 else "Bloqueado"
            )
            df_display = df_display[
                ["login", "nome", "email", "perfil", "criado_em", "ativo"]
            ].rename(
                columns={
                    "login": "Login",
                    "nome": "Nome",
    ...
    with tab3:
        ...
        with st.form("form_alterar_senha"):
            ...
            col_nova, col_confirmar = st.columns(2)
            with col_nova:
            ...
            with col_submit:
                submit_alterar_senha = st.form_submit_button("Alterar senha", width='stretch')

            if submit_alterar_senha:
                conn = sqlite3.connect(DB_PATH)
                c = conn.cursor()
    ...
    with tab4:
        ...
        flash_bloqueio = st.session_state.pop("flash_bloqueio_subordinado", None)
        if flash_bloqueio:
            st.toast(flash_bloqueio["mensagem"], icon=flash_bloqueio["icone"])
        ...
        if not df_usuarios.empty:
            header_cols = st.columns([2, 3, 1.3, 1.2, 1.4])
            header_cols[0].markdown("**Login**")
            header_cols[1].markdown("**Nome**")
            header_cols[2].markdown("**Status**")
            header_cols[3].markdown("**Bloquear**")
            header_cols[4].markdown("**Desbloquear**")
            st.divider()
            ...
            for _, row in df_usuarios.iterrows():
                ...
                ativo = int(row["ativo"])
                login = row["login"]
                nome = row["nome"] if row["nome"] else "-"
                status = "Ativo" if ativo == 1 else "Bloqueado"

                col_login, col_nome, col_status, col_bloquear, col_desbloquear = st.columns(
                    [2, 3, 1.3, 1.2, 1.4]
                )

                col_login.write(login)
                col_nome.write(nome)
                col_status.write(status)

                if col_bloquear.button(
                        "🔒",
                        key=f"bloq_sub_{usuario_id}",
                        width='stretch',
                        disabled=ativo == 0,
                        help=f"Bloquear {login}",
                ):
                    sucesso, mensagem = bloquear_usuario_subordinado(st.session_state.usuario_id, usuario_id)
                    if sucesso:
                        st.session_state.flash_bloqueio_subordinado = {
                            "mensagem": mensagem,
                            "icone": "🔒",
                        }
                        st.rerun()
                ...
                if col_desbloquear.button(
                        "🔓",
                        key=f"debloq_sub_{usuario_id}",
                        width='stretch',
                        disabled=ativo == 1,
                        help=f"Desbloquear {login}",
                ):
    ...
    with tab5:
        ...
        if not df_usuarios.empty:
            ...
            col_select, col_detalhe = st.columns([2, 1])
            with col_select:
                idx = st.selectbox(
                    "Subordinado",
                    range(len(opcoes)),
                    format_func=lambda x: opcoes[x],
                    key="sel_alterar_senha_subordinado",
                )

            selecionado = df_usuarios.iloc[idx]
            usuario_id = int(selecionado['id'])
            login = selecionado['login']
            nome = selecionado['nome'] if selecionado['nome'] else "-"
            status = "Ativo" if int(selecionado['ativo']) == 1 else "Bloqueado"

            with col_detalhe:
                st.markdown("**Usuário selecionado**")
                st.write(f"Login: **{login}**")
                st.write(f"Status: **{status}**")

            with st.form("form_alterar_senha_subordinado"):
                st.caption(f"Redefinindo senha de {nome}")
                ...
                _, col_submit = st.columns([3, 1])
                with col_submit:
                    submit_redefinir = st.form_submit_button("Redefinir senha", width='stretch')
...
def app_painel_admin():
    ...
    with tab1:
        ...
        with st.form("form_gerente"):
            col_empresa, col_doc = st.columns(2)
            with col_empresa:
                nome_empresa = st.text_input("Empresa", placeholder="Nome da empresa")
            with col_doc:
                cnpj_cpf = st.text_input("CNPJ/CPF", placeholder="Somente números")

            col_login, col_senha = st.columns(2)
            with col_login:
                login = st.text_input("Login", placeholder="usuario.gerente")
            with col_senha:
                senha = st.text_input("Senha", type="password", placeholder="Mínimo 6 caracteres")

            email = st.text_input("Email", placeholder="gerente@empresa.com")

            _, col_submit = st.columns([3, 1])

            with col_submit:
                submit_gerente = st.form_submit_button("Criar gerente", width='stretch')
        ...
        if not df_gerentes.empty:
            df_display = df_gerentes.copy()
            df_display["criado_em"] = formatar_data_hora_local(df_display["criado_em"])
            df_display["ativo"] = df_display["ativo"].apply(
                lambda valor: "Ativo" if int(valor) == 1 else "Bloqueado"
            )
            df_display = df_display[
                ["login", "cnpj_cpf", "nome_empresa", "email", "criado_em", "ativo"]
            ].rename(
                columns={
                    "login": "Login",
                    "cnpj_cpf": "CNPJ/CPF",
    ...
    with tab2:
        ...
        col_filtro, col_total = st.columns([1, 2])
        with col_filtro:
            filtro_status = st.selectbox("Status", ["Todos", "Ativos", "Bloqueados"], key="filtro_usuarios")
        ...
        if not df_usuarios.empty:
            with col_total:
                st.metric("Usuários encontrados", len(df_usuarios))
            ...
            df_display = df_display[
                ["login", "cnpj_cpf", "nome", "perfil", "criado_em", "ativo"]
            ].rename(
                columns={
                    "login": "Login",
                    "cnpj_cpf": "CNPJ/CPF",
    ...
    with tab3:
        ...
        tipo_alvo = st.radio("Tipo", ["Gerente", "Usuário Final"], key="tipo_senha", horizontal=True)
        conn = sqlite3.connect(DB_PATH)
        ...
        if tipo_alvo == "Gerente":
            ...
            if not df_gerentes.empty:
                ...
                selecionado = df_gerentes[df_gerentes['login'] == usuario.split(' (')[0]].iloc[0]
                usuario_id = int(selecionado['id'])
                with col_info:
                    st.markdown("**Alvo selecionado**")
                    st.write(f"Login: **{selecionado['login']}**")
                    st.write(f"Empresa: **{selecionado['nome_empresa']}**")

                with st.form("form_senha_gerente"):
                    col_nova, col_conf = st.columns(2)
                    with col_nova:
                        nova = st.text_input(
                            "Nova senha",
                            type="password",
                    ...
                    _, col_submit = st.columns([3, 1])
                    with col_submit:
                        submit_senha = st.form_submit_button("Alterar senha", width='stretch')
            ...
            if not df_usuarios.empty:
                ...
                selecionado = df_usuarios[df_usuarios['login'] == usuario.split(' (')[0]].iloc[0]
                usuario_id = int(selecionado['id'])
                with col_info:
                    st.markdown("**Alvo selecionado**")
                    st.write(f"Login: **{selecionado['login']}**")
                ...
                st.info("Nenhum usuário final cadastrado")
    ...
    with tab4:
        ...
        tipo_bloqueio = st.radio("Tipo", ["Gerente", "Usuário Final"], key="tipo_bloqueio", horizontal=True)
        ...
        if tipo_bloqueio == "Gerente":
            ...
            if not df_gerentes.empty:
                ...
                for _, row in df_gerentes.iterrows():
                    ...
                    ativo = int(row['ativo'])
                    login = row['login']
                    empresa = row['nome_empresa'] if row['nome_empresa'] else "-"
                    status = "Ativo" if ativo == 1 else "Bloqueado"

                    col_login, col_empresa, col_status, col_bloquear, col_desbloquear = st.columns(
                        [2, 3, 1.3, 1.2, 1.4]
                    )
                    col_login.write(login)
                    col_empresa.write(empresa)
                    col_status.write(status)

                    if col_bloquear.button(
                        "🔒",
                        width='stretch',
                        key=f"bloq_gerente_{usuario_id}",
                        disabled=ativo == 0,
                        help=f"Bloquear {login}",
                    ):
                        conn = sqlite3.connect(DB_PATH)
                    ...
                    if col_desbloquear.button(
                        "🔓",
                        width='stretch',
                        key=f"debloq_gerente_{usuario_id}",
                        disabled=ativo == 1,
                        help=f"Desbloquear {login}",
                    ):
                        conn = sqlite3.connect(DB_PATH)
            ...
            if not df_usuarios.empty:
                ...
                for _, row in df_usuarios.iterrows():
                    ...
                    if col_bloquear.button(
                        "🔒",
                        width='stretch',
                        key=f"bloq_usuario_{usuario_id}",
                        disabled=ativo == 0,
                        help=f"Bloquear {login}",
                    ):
                        conn = sqlite3.connect(DB_PATH)
                    ...
                    if col_desbloquear.button(
                        "🔓",
                        width='stretch',
                        key=f"debloq_usuario_{usuario_id}",
                        disabled=ativo == 1,
                        help=f"Desbloquear {login}",
                    ):
                        conn = sqlite3.connect(DB_PATH)
    ...
    with tab5:
        ...
        st.warning("⚠️ Exclusão permanente. Essa ação não pode ser desfeita.")

        tipo_excluir = st.radio("Tipo", ["Gerente", "Usuário Final"], key="tipo_excluir", horizontal=True)
        ...
        if tipo_excluir == "Gerente":
            ...
            if not df_gerentes.empty:
                ...
                col_select, col_resumo = st.columns([2, 1])
                with col_select:
                    idx = st.selectbox(
                        "Gerente",
                        range(len(opcoes)),
                        format_func=lambda x: opcoes[x],
                        key="sel_excluir_gerente",
                    )
                ...
                with col_resumo:
                    ...
                    st.error("Exclui também os usuários subordinados.")

                st.divider()
                st.error(f"🗑️ Confirmar exclusão de **{login}**?")
                conf = st.text_input(
                    f"Digite '{login}' para confirmar",
                    key="conf_excluir_gerente",
                    placeholder=login,
                )
                confirmacao_ok = conf == login

                _, col_button = st.columns([2, 1])
                with col_button:
                    excluir = st.button(
                        "Excluir permanentemente",
                        width='stretch',
                        key=f"exec_excluir_gerente_{usuario_id}",
                        disabled=not confirmacao_ok,
                    )
            ...
            if not df_usuarios.empty:
                ...
                with col_resumo:
                    ...
                    st.error("Exclui apenas este usuário final.")

                st.divider()
                st.error(f"🗑️ Confirmar exclusão de **{login}**?")
                conf = st.text_input(
                    f"Digite '{login}' para confirmar",
                    key="conf_excluir_usuario",
                    placeholder=login,
                )
                confirmacao_ok = conf == login
                ...
                with col_button:
                    excluir = st.button(
                        "Excluir permanentemente",
                        width='stretch',
                        key=f"exec_excluir_usuario_{usuario_id}",
                        disabled=not confirmacao_ok,
                    )
        ...
        with subtab_fin:
            ...
            with subtab_rec1:
                ...
                with st.form("form_conta_recorrente"):
                    ...
                    if st.form_submit_button("✅ Criar Conta Recorrente", width='stretch'):
            ...
            with subtab_rec3:
                ...
                if not df_recorrentes.empty:
                    ...
                    if st.button("⏸️ Desativar", width='stretch', key=f"desativ_{recorrente_id}"):
